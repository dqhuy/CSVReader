...
filename =  strcat(rootpath,'\DataInput\output_industry.csv');
csvwrite(filename,sigma,1,1);
csvwrite(filename,MaxSig,1,4);
csvwrite(filename,Class,1,5);
...
xlswrite(filename,b,1,'B2');  % Luu ket qua tinh he so b 
filename = strcat(rootpath,'\DataOutput\KQ_B.csv');
xlswrite(filename,a,1,'B2');  % Luu ket qua tinh he so a
...
xlswrite(filename,pb,1,'B2');  % Luu ket qua du bao he so b 
pa = pdata(:,5:end);
filename = strcat(rootpath,'\DataOutput\DB_A.csv');
xlswrite(filename,pa,1,'B2');  % Luu ket qua du bao he so a 
...
csvwrite(filename,pgdp,1,'B2');  % Luu ket qua du bao GDP
msg='Done';
end
